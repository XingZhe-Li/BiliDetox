...
class BlockUpdateHook(private val classLoader: ClassLoader) : BaseHook {
    ...
    override val enabled = Config.BLOCK_UPDATE
    ...
    override fun hook() {
        ...
        updateHelper.findMethodOrNull("checkUpdateInStartup", Activity::class.java, iUpdater)
            ?.let { blocked += it.replaceWithNothing() }
    ...
    /** 把方法替换成空实现（返回 null，对 void 方法等价于直接返回）。 */
    private fun java.lang.reflect.Method.replaceWithNothing(): Int {
        XposedBridge.hookMethod(this, XC_MethodReplacement.returnConstant(null))
